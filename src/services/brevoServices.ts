...
const sendBrevoEmail = async (payload: object) => {
  try {
    ...
    logger.log("Email sent: " + response.status);
  } catch (error: any) {
    logger.error("Error sending email", error?.response?.data || error);
    throw error;
  }
};
...
export const sendBrevoInvalidEmail = async (email: string) => {
